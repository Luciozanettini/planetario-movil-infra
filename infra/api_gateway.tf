...
# --- Ticket 5: ruta de estadísticas para el panel de tu papá ---

resource "aws_apigatewayv2_integration" "stats" {
  api_id                 = aws_apigatewayv2_api.leads.id
  integration_type       = "AWS_PROXY"
  integration_uri        = aws_lambda_function.stats_reader.invoke_arn
  payload_format_version = "2.0"
}
...
resource "aws_lambda_permission" "apigw_stats" {
  ...
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.stats_reader.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.leads.execution_arn}/*/*"
}